...
import java.io.*;
class Transaction implements Serializable {String date; int transactionType; int CheckNO; String TransactionsDes; double amount;} 
public class project implements ActionListener {
   ...
   public void actionPerformed(ActionEvent e) {      
      ...
      if (source == deleteSelect) {
         ...
         for (int i = 0;i<Tnumber;++i) {
            data [i][0] = TA [i].date;
            data [i][1] = TransType[TA[i].transactionType];
            data [i][2] = "" + TA [i].CheckNO;
            data [i][3] = TA [i].TransactionsDes;
            if (TA [i].transactionType>=1) {
               data [i][4] = "" + TA[i].amount;
               data [i][5]= "";
               Balance -= TA [i].amount;
            }
            else {
               data [i][5] = "" + TA [i].amount;
               data [i][4]= "";
               Balance += TA [i].amount;
